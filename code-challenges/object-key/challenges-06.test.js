...
const hasChildrenValues = (arr, character) => {
    // Solution code here...
    let haveChiled = false;
    arr.forEach(element => {
        let value = Object.values(element)
            //  console.log(element)  
            //this will give us all properities (keys+values) 
            // console.log(value)
            // this will give us all the values in the array
            // now we need to access all the value to check if the character have a childreen or not!
            // console.log(value[0])
            // the names ^
            // console.log(value[2])
            // the array of the childreens
            // if the array of childreen length is graeter than zero that mean the character have childreen.
        if (value[0] === character && value[2].length > 0) {
            haveChiled = true;
        }
...
const hasChildrenEntries = (arr, character) => {
    // Solution code here...
    var haveChiled = false;
    arr.forEach(element => {
        let value = Object.entries(element);
        if (value[0][1] === character && value[2][1].length > 0) {
            haveChiled = true;
        }
    });
    return haveChiled;
...
const houseSize = (arr) => {
    ...
    // Solution code here...
    arr.forEach(val => {
        let homeArr = Object.values(val);
        let name = 1;
        let spouse = 0;
        if (homeArr[1] !== null) {
            spouse++
        }
        let children = homeArr[2].length;
        let house = homeArr[3];
...
const houseSurvivors = (arr) => {
    ...
    let spouse = 0,
        ...
        house;
    arr.forEach(val => {
        if (val.spouse !== null && !deceasedSpouses.includes(val.spouse)) spouse++;
        house = val.house;
        members = val.children.length + 1 + spouse;
