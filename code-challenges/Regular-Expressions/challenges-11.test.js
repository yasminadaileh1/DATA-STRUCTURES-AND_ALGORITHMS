...
const validateEmail = (email) => {
    // Solution code here...
    let checkEmail = /^\w+(.\w+)@\w+\.(net|com|org)$/g;
...
const validatePhoneNumber = (phoneNumber) => {
    // Solution code here...
    let checkNum = /^(1\s|1|)?((\(\d{3}\))|\d{3})(\-|\s)?(\d{3})(\-|\s)?(\d{4})$/;
    return phoneNumber.match(checkNum);
};
...
const findTagNames = (elements) => {
    ...
    // elements.map(val => {
    //     let matches = val.match(checkTag)
    //     matches.forEach(ele => {
    //         noTag.push(ele)
    //     })
    // })
    ...
    elements.forEach((element) => {
        let check = element.match(checkTag);
        check.map((ele) => {
            noTag.push(ele);
        });
    });
