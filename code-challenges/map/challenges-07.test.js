...
const mapTwoToThe = (arr) => {
    // Solution code here...
    let arr1 = arr.map(value => Math.pow(2, value));
    return arr1;
};
...
const extractAbilities = (arr) => {
    // Solution code here...
    let abilityName = arr.map(element => {
...
const extractStats = (arr) => {
    ...
    let result = [];
    result = arr.map(val => {
        let objName = val.stat.name;
        let objValue = val.effort + val.baseStat;
        return { name: objName, total: objValue };
    });
    return result;
};
