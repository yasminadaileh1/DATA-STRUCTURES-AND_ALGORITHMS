...
const toTitleCase = (arr) => {
    ...
    let arr1 = [];
    arr.forEach(element => {
        let upper = element.charAt(0).toUpperCase() + element.substr(1);
        arr1.push(upper);
    });
    return arr1;
};
...
let biggerThanLuke = (arr) => {
    ...
    let massGreaterThanLuke = Number(starWarsData[0].mass);
    return arr
        .filter(val => Number(val.mass) > massGreaterThanLuke)
        .map(val => val.name)
        .join(' - ');
};
...
const detectTicTacToeWin = (board) => {
    ...
    const CoordinatesForX = getX(board);
    const CoordinatesForO = getO(board);
    const possibleCoordinates = getAllPossible();

    for (let key in possibleCoordinates) {
        const possible = possibleCoordinates[key];
        let count = 0;
        for (let i = 0; i < CoordinatesForX.length; i++) {
            for (let j = 0; j < possible.length; j++) {
                if (
                    CoordinatesForX[i].horizontal == possible[j].horizontal &&
                    CoordinatesForX[i].vertical == possible[j].vertical
                ) {
                    count++;
                }
            }
        ...
        count = 0;
        for (let i = 0; i < CoordinatesForO.length; i++) {
            for (let j = 0; j < possible.length; j++) {
                if (
                    CoordinatesForO[i].horizontal == possible[j].horizontal &&
                    CoordinatesForO[i].vertical == possible[j].vertical
                ) {
                    count++;
                }
            }
