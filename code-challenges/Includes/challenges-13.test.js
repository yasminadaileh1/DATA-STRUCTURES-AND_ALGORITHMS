...
const standardizePhoneNumbers = (arr) => {
    // Solution code here...
    return arr.map(element => `${element.slice(1,4)}${element.slice(6,9)}${element.slice(10)}`);
};
...
const onlyOddChars = (str) => {
    // Solution code here...
    let arr = str.split('');
    let odd = arr.filter((element, value) => {
        if (value % 2 !== 0)
            return element;
    });
    return odd.join('');
...
const unenrollBrook = (arr) => {
    ...
    return arr.map(element => {
        return element.filter(arr2 => !arr2.includes('Brook'))
    });
};
...
const characterByIndex = (arr) => {
    // Solution code here...
    return arr.map((val, i) => val.charAt(i))
};
